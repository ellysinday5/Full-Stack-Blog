...
import { RecentActivityGrid } from "./posts/posts-panel";
...
export function AdminDashboard({ posts, categories }: AdminDashboardProps) {
	...
	// Auto-apply draft filter when redirected from Save as Draft
	useEffect(() => {
		...
		<div className="w-full" onClick={handleBodyClick}>
			...
			<div className="flex items-center gap-3 flex-wrap mb-6">
				...
				>
					...
					{!showForm && <span className="text-lg leading-none">+</span>}
				</button>
			</div>
			...
			<div className="rounded-xl border-2 border-[#1a2e1a]/30 bg-[#f0f3ef] p-5">
				<div className="flex items-center justify-between mb-4">
					<div className="flex items-center gap-2">
						...
						{statusFilter === "draft" && (
							<span className="rounded-full bg-[#c8c8c8] border border-[#b0b0b0] px-2.5 py-0.5 text-[10px] font-bold text-[#3a3a3a]">
								Drafts
							</span>
						)}
					</div>
